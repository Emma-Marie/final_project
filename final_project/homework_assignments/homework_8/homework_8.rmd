---
title: "Homework 8:Sentiment Analysis or Text Mining"
...
### Some wrangling:
 Splitting the pages into separate lines so that each line on each page has its own row. Extra starting & trailing spaces are also removed.
...
got_cloud #viewing word cloud

```

This word cloud isn't that informative, so I will customize it with colors, shape and sizes to give a better view of the most used words:

```{r wordcloud-pro}
wordcloud_got <- ggplot(data = got_top100, aes(label = word, size = n)) +
...
Now its clear that "lord", "ser", "jon", "ned", "hand", "eyes", "told", "bran" and "father" are among the most frequent words (and roughly in that order). 

Saving the plot:
```{r saving plot}
ggsave(plot = wordcloud_got, 
       here("figures","wordcloud_got.png"), 
       height = 8, 
       width = 5)
...
Counting and plotting the -2 words:
...
The analysis shows that the bin with the most words are the "positive" bin. But the "negative" bin is almost as big, and a lot of other bins with negative connotations such as "anger", "disgust" and "fear" are also containing a lot of words. So the positive bin seems to be outnumbered by the bins with a more negative connotation. 
...
The word "lord" is ambiguous, because it occurs four of the bins, so the word is both binned as "disgust", "negative", "positive" and "trust".
...
Some were some things that surprises me from the chart:
...
- Why is "blue" sad? Sometimes its just a color without any symbolic meaning.
