...
References: 
Walter, Tony (2015), ”Secularization”, In: In: C.M. Parkes & W. Young, eds Death and Bereavement Across Cultures, London: Routledge, 2nd edn, 2015, pp. 133-148.
...
After viewing the data frame, I see that every single piece of data has been separated into individual columns under the right year, which was my intention. But column one still contains all the information from all years bundled up in one column. This column is now needless and I will remove it:
...
Now I remove the unnecessary rows: row 2 which contain the years, row 1 which contain a comment about the data unit, row 10 which is empty and row 11 which contains a comment on the source of the data: 
...
To be able to do some visualization with percentages, I remove the `registered deaths` row and sum all the funerals together in a new row:
...
I will create a data frame only containing the religious funerals and the sum of those. This means I will remove the `registered deaths` row and the `civil funeral` row from the `funerals_clean` data and then sum the rest of the rows:

```{r remove row 1 and 7, results=FALSE}
funerals_religious <- funerals_clean[-c(1,7),] #removing civil funerals and registered deaths

funerals_religious # the rows have been removed
...
funerals_rel_total <- funerals_religious %>%
  bind_rows(summarise_all(., ~if(is.numeric(.)) sum(.) else "total religious funerals"))

```
Saving the data frame with the total amount of religious funerals in a csv called `funerals_religious.csv`:

```{r save total religious funerals data frame}
setwd('..') #changing position
...
funerals_nationalchurch <- funerals_noregdeaths[1:4,] #choosing the foyr relevant columns
...
Save th total amount of funerals in danish national church, total amount of funerals in free churches and other religions, and the total number of civil funerals in the same data frame:

```{r data frame all total values}
#Adding rows from other data sets
total_allfuneraltypes <- funerals_rel_total[5,] #adding free church and other religions

total_allfuneraltypes[nrow(total_allfuneraltypes) + 1,] <- funerals_nationalchurch_total[5,] #adding national church total

total_allfuneraltypes[nrow(total_allfuneraltypes) + 1,] <- funerals_total[6,] #adding national church total
...
- small decline in civil funerals form 2018-2021. In the same period funerals in both the national church and in other religions or free churches are increased.
...
-  The civil funerals has increased from 8,7% in 2006 to 13,8% in 2021. 
-  The funerals in free churches and other religions have increased from 1,5% in 2006 to 5.2% in 2021
-  The funerals in the national church has decreased from 89,8% in 2006 to 81% in 2021. 
-  There are more civil funerals than funerals in free churches and other religions in both 2006 and 2021. 


## Conclusion

From 2006-2021 there has been a small increase in civil funerals, but it is indeed very small. There has also been a small decline in funerals in the national church, but the national church still hold the great majority of funerals in Denmark. 

There were a small decline in civil funerals since 2018-2021, and both funerals in national church and in other religions/free churches increased in the same period.
